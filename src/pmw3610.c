...
static uint32_t automouse_current_timeout_ms = CONFIG_PMW3610_AUTOMOUSE_TIMEOUT_MS;
...
static void shorten_automouse_timeout(void) {
    set_automouse_timeout(CONFIG_PMW3610_AUTOMOUSE_TIMEOUT_MS);
}

static bool should_control_automouse_layer(void) {
    return automouse_triggered || !is_automouse_layer_active();
}

static uint32_t movement_size(int16_t x, int16_t y) { return abs(x) + abs(y); }
...
static void handle_automouse_layer_state_change(void) {
    static bool last_automouse_layer_state;
    bool automouse_layer_active = is_automouse_layer_active();

    if (!last_automouse_layer_state && automouse_layer_active) {
        reset_automouse_state();
    } else if (last_automouse_layer_state && !automouse_layer_active) {
        automouse_triggered = false;
        shorten_automouse_timeout();
        reset_automouse_state();
    ...
    last_automouse_layer_state = automouse_layer_active;
}

static bool should_activate_automouse_layer(enum pixart_input_mode input_mode, int16_t x, int16_t y) {
    if (input_mode != MOVE || !should_control_automouse_layer()) {
        return false;
    }

    automouse_total_movement += movement_size(x, y);
    if (automouse_total_movement <= CONFIG_PMW3610_MOVEMENT_THRESHOLD) {
        return false;
    }

    reset_automouse_state();
    keep_automouse_timeout();
    return true;
}
#endif
...
#if AUTOMOUSE_LAYER > 0
            if (should_activate_automouse_layer(input_mode, x, y)) {
...
#if AUTOMOUSE_LAYER > 0
static int pmw3610_handle_listener_event(const zmk_event_t *eh) {
    if (as_zmk_layer_state_changed(eh) != NULL) {
        handle_automouse_layer_state_change();
    }

    const struct zmk_mouse_button_state_changed *mouse_button_ev =
        as_zmk_mouse_button_state_changed(eh);
    if (mouse_button_ev != NULL && mouse_button_ev->state && automouse_triggered) {
        shorten_automouse_timeout();
        reset_automouse_state();
        if (is_automouse_layer_active()) {
            k_timer_start(&automouse_layer_timer, K_MSEC(automouse_current_timeout_ms), K_NO_WAIT);
...
ZMK_LISTENER(pmw3610_listener, pmw3610_handle_listener_event);
ZMK_SUBSCRIPTION(pmw3610_listener, zmk_layer_state_changed);
ZMK_SUBSCRIPTION(pmw3610_listener, zmk_mouse_button_state_changed);
#endif
